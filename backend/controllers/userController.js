  ...
  try {
    ...
    try {
      ...
      if (calendarificResponse.data.response.holidays?.length > 0) {
        return res.status(400).json({ message: 'Last working day cannot be on a holiday' })
      }
    } catch (apiError) {
      console.error('Calendarific API error:', apiError)
    }
...
export const submitExitResponses = async (req, res) => {
  try {
    ...
    res.json({ message: 'Exit interview responses submitted successfully' })
  } catch (error) {
